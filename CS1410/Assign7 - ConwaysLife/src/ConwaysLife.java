...
public class ConwaysLife {
    public static void main(String[] args) {
        try {
            ...
            simulation.insertPattern(new PatternForFun(),0,0); //I added this part as well************
            screen.startScreen();
    ...
    public static void sampleRender(Screen screen, TextGraphics graphics, int xPos) {
        ...
        graphics.setCharacter(xPos, 20, 'X');
    ...
    public static void render(LifeSimulator simulation, Screen screen, TextGraphics graphics){
        screen.clear();
        boolean[][] world = simulation.getWorld();
        for(int i = 0; i < world.length; i++){
            for(int j = 0; j < world[i].length; j++){
                if(world[i][j]){graphics.setCharacter(i, j, 'x');}
            }
        }
        try {
            screen.refresh();
